...
BROKER_IP = "localhost"
BROKER_PORT = 61613
FILA_DESPACHO = "/queue/despacho.ambulancia"
...
def consultar_servidor_nomes(nome_busca):
    ...
    try:
        ...
        print(f"[ERRO] Não foi possível consultar o Servidor de Nomes: {e}")
        return None
...
def conectar_broker():
    try:
        ...
        print(f"[AMBULÂNCIA] Conectada ao ActiveMQ em {BROKER_IP}:{BROKER_PORT}")
        return conn
    except Exception as e:
...
def publicar_status_broker(conn_broker, evento, id_incidente):
    try:
        ...
        print(f"[AMBULÂNCIA] STATUS publicado no broker | Incidente={id_incidente} | {evento}")
    except Exception as e:
        print(f"[ERRO] Falha ao publicar status no ActiveMQ: {e}")


class ListenerAmbulancia(stomp.ConnectionListener):
    def on_error(self, frame):
        print(f"[AMBULÂNCIA][BROKER][ERRO] {frame.body}")
    ...
    def on_message(self, frame):
        ...
        try:
            ...
            print("[AMBULÂNCIA] Mensagem inválida recebida do ActiveMQ, ignorando...")
            return
        ...
        if msg.get("tipo_mensagem") == "COMANDO" and msg.get("acao") == "ATENDER_INCIDENTE":
            ...
            print(
                f"[AMBULÂNCIA] COMANDO recebido via ActiveMQ -> "
...
def enviar_evento_operacional(cliente, conn_broker, evento, id_incidente):
    ...
    print(f"[AMBULÂNCIA] EVENTO enviado | Incidente={id_incidente} | {evento} | Lamport={lamport_clock}")
...
def executar_atendimento(cliente, conn_broker, chamado):
    ...
    print(
        f"[AMBULÂNCIA] Novo chamado recebido -> "
        f"Incidente={id_incidente} | Tipo={tipo_incidente} | "
        f"Local=({coordenadas.get('x')},{coordenadas.get('y')})"
    )

    eventos = [
        f"Deslocamento iniciado para o incidente {id_incidente}.",
        f"Chegada ao local do incidente {id_incidente}.",
        f"Atendimento médico iniciado no incidente {id_incidente}.",
        f"Paciente estabilizado no incidente {id_incidente}.",
        f"Atendimento finalizado no incidente {id_incidente}."
    ]
    ...
    msg_status = {
        "tipo_mensagem": "EVENTO",
        "origem": NOME_ENTIDADE,
        "conteudo": f"Unidade médica disponível novamente após atendimento do incidente {id_incidente}.",
        "id_incidente": id_incidente,
        "timestamp": time.ctime(),
        "lamport": lamport_clock,
        "vetor": list(vector_clock)
    ...
    publicar_status_broker(
        conn_broker,
        f"Unidade médica disponível novamente após atendimento do incidente {id_incidente}.",
        id_incidente
    )

    print(f"[AMBULÂNCIA] Incidente {id_incidente} concluído. Unidade voltou a aguardar chamados.")
...
#INICIALIZAÇÃO
def iniciar_ambulancia():
    global lider_atual, aguardando_grant, cliente_global

    print("[AMBULÂNCIA] Localizando Central de Coordenação...")

    info_central = consultar_servidor_nomes(NOME_CENTRAL)

    if not info_central or "erro" in info_central:
        print("[ERRO] Central não encontrada no registro.")
        return
    ...
    if conn_broker is None:
        print("[ERRO] Ambulância não conseguiu iniciar sem ActiveMQ.")
        return

    try:
        cliente.connect((ip_central, porta_central))
        print(f"[AMBULÂNCIA] Conectada à Central em {ip_central}:{porta_central}")
        print("[AMBULÂNCIA] Status: unidade médica disponível e aguardando chamados.")

        incrementar_relogio_local()
        msg_online = {
            "tipo_mensagem": "EVENTO",
            "origem": NOME_ENTIDADE,
            "conteudo": "Unidade médica online e aguardando chamados.",
            "timestamp": time.ctime(),
            "lamport": lamport_clock,
            "vetor": list(vector_clock)
        }
        enviar_seguro(cliente, msg_online)
        publicar_status_broker(conn_broker, "Unidade médica online e aguardando chamados.", None)

        listener = ListenerAmbulancia()
        ...
        print(f"[AMBULÂNCIA] Aguardando comandos do broker em {FILA_DESPACHO}...")
        ...
        while True:
            ...
            if not data:
                print("[AMBULÂNCIA] Conexão encerrada pela Central.")
                break
            ...
            while "\n" in buffer:
                ...
                try:
                    ...
                    print("[AMBULÂNCIA] Mensagem inválida recebida, ignorando...")
                    continue
                ...
                if tipo == "GRANT_SC":
                    ...
                    print(f"[AMBULÂNCIA] Novo líder reconhecido: {lider_atual}")

                else:
                    print(f"[AMBULÂNCIA] Msg de {msg.get('origem', 'DESCONHECIDO')}: {msg.get('conteudo', '')}")
        ...
        print(f"[ERRO] Falha na comunicação da Ambulância: {e}")
