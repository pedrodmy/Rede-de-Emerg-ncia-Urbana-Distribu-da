...
BROKER_IP = "localhost"
BROKER_PORT = 61613
FILA_DESPACHO = "/queue/despacho.bombeiros"
...
def conectar_broker():
    try:
        ...
        print(f"[BOMBEIROS] Conectados ao ActiveMQ em {BROKER_IP}:{BROKER_PORT}")
        return conn
    except Exception as e:
...
def publicar_status_broker(conn_broker, evento, id_incidente):
    try:
        ...
        print(f"[BOMBEIROS] STATUS publicado no broker | Incidente={id_incidente} | {evento}")
    except Exception as e:
        print(f"[ERRO] Falha ao publicar status no ActiveMQ: {e}")


class ListenerBombeiros(stomp.ConnectionListener):
    def on_error(self, frame):
        print(f"[BOMBEIROS][BROKER][ERRO] {frame.body}")
    ...
    def on_message(self, frame):
        ...
        try:
            ...
            print("[BOMBEIROS] Mensagem inválida recebida do ActiveMQ, ignorando...")
            return
        ...
        if msg.get("tipo_mensagem") == "COMANDO" and msg.get("acao") == "ATENDER_INCIDENTE":
            ...
            print(
                f"[BOMBEIROS] COMANDO recebido via ActiveMQ -> "
...
def enviar_evento_operacional(cliente, conn_broker, evento, id_incidente):
    ...
    print(f"[BOMBEIROS] EVENTO enviado | Incidente={id_incidente} | {evento} | Lamport={lamport_clock}")
...
def executar_atendimento(cliente, conn_broker, chamado):
    ...
    print(
        f"[BOMBEIROS] Novo chamado recebido -> "
    ...
    eventos = [
        ...
        f"Combate operacional iniciado no incidente {id_incidente}.",
        f"Situação crítica controlada no incidente {id_incidente}.",
        f"Rescaldo e verificação final em andamento no incidente {id_incidente}.",
        f"Atendimento dos bombeiros finalizado no incidente {id_incidente}."
    ]
    ...
    msg_status = {
        "tipo_mensagem": "EVENTO",
        "origem": NOME_ENTIDADE,
        "conteudo": f"Equipe de bombeiros disponível novamente após atendimento do incidente {id_incidente}.",
        "id_incidente": id_incidente,
        "timestamp": time.ctime(),
        "lamport": lamport_clock,
        "vetor": list(vector_clock)
    ...
    publicar_status_broker(
        conn_broker,
        f"Equipe de bombeiros disponível novamente após atendimento do incidente {id_incidente}.",
        id_incidente
    )

    print(f"[BOMBEIROS] Incidente {id_incidente} concluído. Equipe voltou a aguardar chamados.")
...
#INICIALIZAÇÃO
def iniciar_bombeiros():
    global lider_atual, aguardando_grant

    print("[BOMBEIROS] Localizando Central de Coordenação...")
    ...
    if conn_broker is None:
        print("[ERRO] Bombeiros não conseguiram iniciar sem ActiveMQ.")
        return

    try:
        cliente.connect((ip_central, porta_central))
        print(f"[BOMBEIROS] Conectados à Central em {ip_central}:{porta_central}")
        print("[BOMBEIROS] Status: equipe ativa e aguardando acionamento.")

        incrementar_relogio_local()
        msg_online = {
            "tipo_mensagem": "EVENTO",
            "origem": NOME_ENTIDADE,
            "conteudo": "Equipe de bombeiros ativa e aguardando chamados.",
            "timestamp": time.ctime(),
            "lamport": lamport_clock,
            "vetor": list(vector_clock)
        }
        enviar_seguro(cliente, msg_online)
        publicar_status_broker(conn_broker, "Equipe de bombeiros ativa e aguardando chamados.", None)

        listener = ListenerBombeiros()
        ...
        print(f"[BOMBEIROS] Aguardando comandos do broker em {FILA_DESPACHO}...")
        ...
        while True:
            ...
            if not data:
                print("[BOMBEIROS] Conexão encerrada pela Central.")
                break
            ...
            while "\n" in buffer:
                ...
                try:
                    ...
                    print("[BOMBEIROS] Mensagem inválida recebida, ignorando...")
                    continue
                ...
                if tipo == "GRANT_SC":
                    ...
                    print(f"[BOMBEIROS] Novo líder reconhecido: {lider_atual}")

                else:
                    print(f"[BOMBEIROS] Msg de {msg.get('origem', 'DESCONHECIDO')}: {msg.get('conteudo', '')}")
        ...
        print(f"[ERRO] Falha na comunicação dos Bombeiros: {e}")
